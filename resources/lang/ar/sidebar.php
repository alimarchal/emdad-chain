<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Sidebar Arabic Language Words
    |--------------------------------------------------------------------------
    |
    */

    "Buyer" => 'مشتري',
    "Supplier" => 'مورّد',
    "SuperAdmin" => 'المشرف الرئيسي',
    'Virtual office' => 'المكتب الافتراضي',
    'Buyer Virtual office' => 'المكتب الافتراضي للمشتري',
    'Supplier Virtual office' => 'المكتب الافتراضي للمورّد',
    'Super Admin Virtual office' => 'المكتب الافتراضي للإدارة الحساب',
    'Home' => 'الرئيسية',
    'Roles' => 'الوظائف',
    'Permissions' => 'التصريحات',
    'Warehouse' => 'المستودع',
    'Add Vehicle' => 'إضافة عربة',
    'List of Vehicles' => 'قائمة العربات',
    'Businesses' => 'الأنشطة التجارية',
    'Businesses Certificates' => 'وثائق النشاط التجاري',
    'Incomplete Registration' => 'التسجيل غير مكتمل',
    'Downloads' => 'التنزيلات',
    'Commission percentage' => 'نسبة العمولة',
    'IREs' => 'ممثلو إمداد المستقلون',
    'Ratings' => 'التقييم',
    'SMS' => 'الرسائل النصية',
    'RFQs' => 'الطلبات',
    'Quotations' => 'عروض الأسعار',
    'Multi categories' => 'الفئات المتعددة',
    'Single category' => 'الفئة الفردية',
    'Purchase Order' => 'أمر شراء',
    'Purchase Orders' => 'طلبات الشراء',
    'Delivery' => 'التوصيل',
    'Create Delivery Note' => 'إنشاء مذكرة توصيل',
    'DN history' => 'سجل مذكرات التوصيل',
    'Shipments' => 'الشحنات',
    'Generate' => 'إنشاء',
    'Cart' => 'سلة',
    'Placed Shipments' => 'الشحنات الخارجة للتورد',
    'Payments' => 'المدفوعات',
    'Manual Payments' => 'الدفع الذاتي',
    'Payments to supplier' => 'الدفعات للمورّد',
    'Emdad Invoices' => 'فواتير إمداد',
    'Invoices' => 'الفواتير',
    'Current Shipments' => 'الشحنات الحالية',
    'Delivered Shipments' => 'الشحنات المستلمة',
    'Shipments Histroy' => 'سجل الشحنات',
    'Buyers' => 'المشترون',
    'Suppliers' => 'المورّدون',
    'Add Buyer' => 'إضافة مشتري',
    'Add Supplier' => 'إضافة مورّد',
    'List of Buyers' => 'قائمة المشترين',
    'List of Suppliers' => 'قائمة المورّدين',
    'Delivery Note' => 'مذكرة توصيل',
    'Add Driver' => 'إضافة سائق',
    'Received Shipments' => 'الشحنات المستلمة',
    'Package Payments' => 'مدفوعات الباقة',

];
