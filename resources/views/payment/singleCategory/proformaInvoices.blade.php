...
@if (auth()->user()->rtl == 0)
    <x-app-layout>
        <x-slot name="header">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight"> {{ __('Delivery Note') }} </h2>
        </x-slot>
                                        ...
                                        <table id="proforma-table" class="min-w-full divide-y divide-gray-200">
                                            ...
                                            @foreach ($dpos as $dpo)
                                                <tr>
                                                    ...
                                                    <td class="px-6 py-4 text-center whitespace-nowrap text-sm text-black">
                                                        @if($dpo->payment_term == 'Cash' || auth()->user()->can('all') && auth()->user()->registration_type == 'Supplier' && auth()->user()->status == 3)
                                                            ...
                                                            @if(isset($deliveryNote) && $deliveryNote->status != 'completed' )
                                                                <a>{{__('portal.Generate Final Invoice')}}</a>
                                                            @elseif(isset($deliveryNote) && $deliveryNote->status == 'completed' )
                                                                <a>{{__('portal.Final Invoice Generated')}}</a>
                                                    ...
                                                    <td class="px-6 py-4 text-center whitespace-nowrap text-sm text-black">
                                                        <a class="hover:text-blue-900 text-blue-900">@if($dpo->status == 'prepareDelivery') {{__('portal.Preparing delivery')}} @else {{$dpo->status}} @endif</a>
                                                    </td>
