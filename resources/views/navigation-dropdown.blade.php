            ...
            <div class="flex justify-between h-16">
                <div class="flex">
                    ...
                    <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                        ...
                        @endrole
                        @role('SuperAdmin')
                        <x-jet-nav-link href="{{ route('category.create') }}"
                        ...
                        @role('SC Specialist')
                        <x-jet-nav-link href="{{ route('showAllCategory') }}"
                                        :active="request()->routeIs('showAllCategory')">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                 xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                      d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>
                ...
                <div class="hidden sm:flex sm:items-center sm:ml-6">
                    ...
                    <span title="{{__('navigation-dropdown.Number of Requisitions which received no quotations')}}">{{__('navigation-dropdown.Requisitions(s) with no Quotations')}}: &nbsp;<a
                            href="{{route('RFQsWithNoQuotations')}}" style="padding-right: 10px;"><span
                                class="text-blue-600 hover:underline">{{$rfqs}}</span></a></span>
                    @endrole
                    ...
                    @if(auth()->user()->hasRole('CEO') && auth()->user()->registration_type == 'Supplier' && auth()->user()->status == 3)
                        ...
                        <span
                            title="{{__('navigation-dropdown.Number of New Requisitions(s) received against multiple categories')}}">{{ __('navigation-dropdown.Multiple Categories RFQ')}}: &nbsp;<a
                                href="{{route('viewRFQs')}}" style="padding-right: 10px;"><span
                        ...
                        <span
                            title="{{__('navigation-dropdown.Number of New Requisitions(s) received against single category')}}">{{ __('navigation-dropdown.Single Category RFQ')}}: &nbsp;<a
                                href="{{route('singleCategoryRFQs')}}" style="padding-right: 10px;"><span
                    ...
                    @if(auth()->user()->hasRole('CEO|Buyer Create New RFQ') && auth()->user()->registration_type == 'Buyer' && auth()->user()->status == 3)
                        ...
                        @endphp
                        <span title="{{__('navigation-dropdown.Number of New Quotation(s) received')}}"
                              style="margin-left: 10px;">{{ __('navigation-dropdown.Quotations')}}: &nbsp;<a
                                href="{{route('QoutationsBuyerReceived')}}" style="padding-right: 10px;"><span
                            ...
                            @elseif(auth()->user()->usertype == "CEO" && auth()->user()->registration_type == "Supplier")
                                ...
                                @foreach(Auth::user()->unreadNotifications as $notification)
                                    <a href="@if(isset($notification->data['url_route'])){{route('notificationShow',$notification->id)}}@else # @endif" class="flex items-center px-4 py-3 text-gray-600 hover:text-white hover:bg-indigo-600 -mx-2">
                                        <p class="text-sm mx-2">
                                            <span class="font-bold">@if(isset($notification->data['notification_data'])) {{$notification->data['notification_data']}} @endif
                                            </span>
                                        </p>
                                    </a>
                                @endforeach
                    ...
                    <x-jet-dropdown align="right" width="48">
                        ...
                        <x-slot name="content">
                            ...
                            @can('all')
                                <div class="border-t border-gray-100"></div>
                                <x-jet-responsive-nav-link href="{{ route('log.viewer') }}"
                                                           :active="request()->routeIs('log.viewer')">
                                    {{ __('navigation-dropdown.Log Viewer') }}
                                </x-jet-responsive-nav-link>

                                <div class="border-t border-gray-100"></div>
                                <x-jet-responsive-nav-link href="{{ route('user_logs') }}"
                                                           :active="request()->routeIs('user_logs')">
                            ...
                            @if (auth()->user()->hasRole('CEO') && Auth::user()->registration_type != null)
                                ...
                                @if (isset($isBusinessPOIExist))
                                    <x-jet-dropdown-link
                                        href="{{ route('purchaseOrderInfo.show', $isBusinessPOIExist->id) }}">
                            ...
                            @if (auth()->user()->usertype != 'CEO')
                                <x-jet-dropdown-link href="{{ route('policyProcedure.eula') }}" target="_blank"
                                                     :active="request()->routeIs('policyProcedure.eula')">
                            ...
                            @if (auth()->user()->usertype == 'CEO')
                                @if(auth()->user()->registration_type == 'Buyer')
                                    <x-jet-dropdown-link href="{{ route('policyProcedure.buyer') }}" target="_blank"
                                                         :active="request()->routeIs('policyProcedure.buyer')">
                                        {{ __('navigation-dropdown.Policy and Procedure') }}
                                    </x-jet-dropdown-link>
                                @elseif(auth()->user()->registration_type == 'Supplier')
                                    <x-jet-dropdown-link href="{{ route('policyProcedure.supplier') }}" target="_blank"
                                                         :active="request()->routeIs('policyProcedure.supplier')">
                                        {{ __('navigation-dropdown.Policy and Procedure') }}
                                    </x-jet-dropdown-link>
                            @endif
                        @endif
        ...
        <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
            <div class="pt-2 pb-3 space-y-1">
                ...
                @if(auth()->user()->hasRole('CEO|Buyer Create New RFQ') && auth()->user()->registration_type == 'Buyer' && auth()->user()->status == 3)
                        ...
                        <div style="float:left; text-align:left; display:inline;">
                            @if($multipleCategoryCount > 0)
                                <img src="{{asset('cart.png')}}"
                                     style="margin-left: 10px;margin-bottom: 4px;width: 32px; height: 32px;">
                                <a href="{{route('RFQCart.index')}}"
                                   title="{{__('navigation-dropdown.Multiple Categories Cart items')}}"
                    ...
                    @if($multipleCategoryCount == 0 && $count == 0) @elseif($multipleCategoryCount == 0 || $count == 0)
                        <br><br><br> @endif
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                ...
                <div class="hidden sm:flex sm:items-center sm:ml-6">
                    ...
                    @php $rfqs = \App\Models\EOrderItems::with('qoutes')->doesntHave('qoutes')->count(); @endphp
                    <span title="{{__('navigation-dropdown.Number of Requisitions which received no quotations')}}">{{__('navigation-dropdown.Requisitions(s) with no Quotations')}}: &nbsp;<a
                            href="{{route('RFQsWithNoQuotations')}}" style="padding-left: 10px;"><span
                                class="text-blue-600 hover:underline">{{$rfqs}}</span></a></span>
                    @endrole
                    ...
                    @if(auth()->user()->hasRole('CEO|Buyer Create New RFQ') && auth()->user()->registration_type == 'Buyer' && auth()->user()->status == 3)
                        ...
                        @if($multipleCategoryCount > 0)
                            <img src="{{asset('cart.png')}}"
                                 style="transform: scaleX(-1) ;margin-left: 10px;margin-bottom: 6px;"> &nbsp;
                            <a href="{{route('RFQCart.index')}}"
                               title="{{__('navigation-dropdown.Multiple Categories Cart items')}}">
                                <span @if($multipleCategoryCount > 0) class="text-green-600 hover:underline"
                        ...
                        @if($count > 0)
                            <img src="{{asset('cart2.png')}}"
                                 style="transform: scaleX(-1) ;margin-right: 10px;margin-bottom: 4px;"> &nbsp;
                            <a href="{{route('single_cart_index')}}"
                               title="{{__('navigation-dropdown.Single Category Cart items')}}">
                                <span @if($count > 0) class="text-green-600 hover:underline"
                                      @else class="text-red-600 hover:underline" @endif style="font-family: sans-serif">
                    ...
                    <x-jet-dropdown align="left" width="48">
                        ...
                        <x-slot name="content">
                            ...
                            @endif
                            @if (auth()->user()->usertype != 'CEO')
                                <x-jet-dropdown-link href="{{ route('arabic.policyProcedure.eula') }}" target="_blank"
                                                     :active="request()->routeIs('policyProcedure.eula')">
                                    {{ __('navigation-dropdown.Policy and Procedure') }}
                                </x-jet-dropdown-link>
                            @endif
                            ...
                            @if (auth()->user()->usertype == 'CEO')
                                @if(auth()->user()->registration_type == 'Buyer')
                                    <x-jet-dropdown-link href="{{ route('arabic.policyProcedure.buyer') }}"
                                                         target="_blank"
                                                         :active="request()->routeIs('policyProcedure.buyer')">
                                        {{ __('navigation-dropdown.Policy and Procedure') }}
                                    </x-jet-dropdown-link>
                                @elseif(auth()->user()->registration_type == 'Supplier')
                                    <x-jet-dropdown-link href="{{ route('arabic.policyProcedure.supplier') }}"
                                                         target="_blank"
                                                         :active="request()->routeIs('policyProcedure.supplier')">
                                        {{ __('navigation-dropdown.Policy and Procedure') }}
                                    </x-jet-dropdown-link>
        ...
        <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
            ...
            <div class="pt-4 pb-1 border-t border-gray-200">
                ...
                <div class="mt-3 space-y-1">
                    ...
                    @if (auth()->user()->usertype == 'CEO')
                        @if(auth()->user()->registration_type == 'Buyer')
                            <x-jet-dropdown-link href="{{ route('arabic.policyProcedure.buyer') }}" target="_blank"
                                                 :active="request()->routeIs('policyProcedure.buyer')">
                                {{ __('navigation-dropdown.Policy and Procedure') }}
                            </x-jet-dropdown-link>
                        @elseif(auth()->user()->registration_type == 'Supplier')
                            <x-jet-dropdown-link href="{{ route('arabic.policyProcedure.supplier') }}" target="_blank"
                                                 :active="request()->routeIs('policyProcedure.supplier')">
                                {{ __('navigation-dropdown.Policy and Procedure') }}
                            </x-jet-dropdown-link>
                        @endif
