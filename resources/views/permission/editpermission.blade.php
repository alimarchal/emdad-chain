<x-app-layout>
    ...
    <div class="flex flex-col bg-white rounded mt-8">
        <form action="{{ route('permission.update', $permission->id) }}" method="post" class="form bg-white p-6" enctype="multipart/form-data">
            ...
            @method('PUT')
            @method('PATCH')
            <h3 class="text-2xl text-gray-900 font-semibold text-center">Update permission</h3>
            <p class="text-gray-600">Permession Name</p>


            <div class="flex space-x-5 mt-3">
                <input type="text" name="name" id="" class="border p-2 min-w-full" value="{{ $permission->name }}">
    ...
    <div class="mt-5">
        ...
        <a href="{{ route('permission.create') }}" style="background-color: #145EA8"
