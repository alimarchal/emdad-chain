...
@section('body')
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                                    ...
                                    <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                                        <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                                            <table class="min-w-full divide-y divide-gray-200" id="roles-table">
                                                ...
                                                @foreach ($incompleteReferences as $incompleteReference)
                                                    <tr>
                                                        <td class="px-6 py-4 whitespace-nowrap">
                                                            <div class="text-sm text-center text-gray-900">{{ $loop->iteration }}</div>
                                                        </td>
                                                        <td class="px-6 py-4 whitespace-nowrap">
                                                            <div class="text-sm text-center text-gray-900">{{ $incompleteReference->nonIreReference->name }}</div>
                                                        </td>
                                                        <td class="px-6 py-4 whitespace-nowrap">
                                                            <div class="text-sm text-center text-gray-900">{{ $incompleteReference->nonIreReference->email }}</div>
                                                        </td>
                                                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                                            <div class="text-sm text-center text-gray-900">{{ $incompleteReference->nonIreReference->mobile }}</div>
                                                        </td>
                                                    </tr>
            ...
            @if ($incompleteReferences->count() >= 50)
                <div class="px-4 py-5 mt-3 bg-white sm:p-6 rounded-sm">
                    <div class="col-span-12 sm:col-span-12">
                        {{ $incompleteReferences->links() }}
                    </div>
                </div>
            @endif
        </div>
