        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    ...
                    <div class="py-3">
                        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                            <h2 class="text-2xl font-bold text-center">{{__('portal.Emdad Invoices')}}</h2>
                                                        ...
                                                        @foreach ($emdadInvoices as $emdadInvoice)
                                                            <tr>
                                                                ...
                                                                <td class="whitespace-nowrap ml-10" style="text-align:center;">
                                                                    <span>@if($emdadInvoice->invoice->invoice_type == 1) {{__('portal.Proforma Invoice')}} @else {{__('portal.Final Invoice')}} @endif</span>
                                                                </td>

                                                                <td class="whitespace-nowrap ml-10" style="text-align:center;">
                                                                    <span>
                                                                        @if($emdadInvoice->invoice->invoice_status == 0) {{__('portal.Un-Paid')}}
                                                                        @elseif($emdadInvoice->invoice->invoice_status == 1) {{__('portal.Supplier Verification Pending')}}
                                                                        @elseif($emdadInvoice->invoice->invoice_status == 2) {{__('portal.Supplier Rejected')}}
                                                                        @elseif($emdadInvoice->invoice->invoice_status == 3) {{__('portal.Payment received')}}
                                                                        @endif
                                                                    </span>
                                                                </td>
                                                                <td class="whitespace-nowrap ml-10" style="text-align:center;">
                                                                    @if($emdadInvoice->invoice->invoice_status == 3 && $emdadInvoice->invoice->invoice_type == 0 && $emdadInvoice->send_status == 0)
                                                                        <form action="{{route('singleCategoryGenerateInvoice')}}" method="POST" class="inline-flex items-center justify-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-500 focus:outline-none focus:border-blue-700 focus:shadow-outline-blue active:bg-blue-600 transition ease-in-out duration-150">
                                                                            @csrf
                                                                            @foreach($IDs as $ID)
                                                                                <input type="hidden" name="IDs[]" value="{{$ID}}">
                                                                            @endforeach
                                                                            <button type="submit">{{__('portal.Send Emdad Invoice')}}</button>
                                                                        </form>
                                                                    ...
                                                                    @elseif($emdadInvoice->invoice->invoice_type == 1)
                                                                        <a @if($emdadInvoice->invoice->invoice_status == 2) class="inline-flex items-center justify-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500 focus:outline-none focus:border-red-700 focus:shadow-outline-red active:bg-red-600 transition ease-in-out duration-150" @else class="inline-flex items-center justify-center px-4 py-2 bg-yellow-400 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-500 focus:outline-none focus:border-yellow-700 focus:shadow-outline-yellow active:bg-yellow-600 transition ease-in-out duration-150" @endif>
                                                                            @if($emdadInvoice->invoice->invoice_status == 0) {{__('portal.Proforma Invoice Un-paid')}}
                                                                            @elseif($emdadInvoice->invoice->invoice_status == 1) {{__('portal.Proforma invoice verification Pending')}}
                                                                            @elseif($emdadInvoice->invoice->invoice_status == 2) {{__('portal.Proforma invoice rejected')}}
                                                                            @elseif($emdadInvoice->invoice->invoice_status == 3) {{__('portal.Proforma invoice accepted')}}
                                                                            @endif
                                                                        </a>
                                                        ...
                                                        @foreach ($emdadInvoices as $emdadInvoice)
                                                            <tr>
                                                                ...
                                                                <td class="whitespace-nowrap ml-10" style="text-align:center;">
                                                                    ...
                                                                    @elseif($emdadInvoice->invoice->invoice_type == 1)
                                                                        <a @if($emdadInvoice->invoice->invoice_status == 2) class="inline-flex items-center justify-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500 hover:text-white focus:outline-none focus:border-red-700 focus:shadow-outline-red active:bg-red-600 transition ease-in-out duration-150" @else class="inline-flex items-center justify-center px-4 py-2 bg-yellow-400 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-500 hover:text-white focus:outline-none focus:border-yellow-700 focus:shadow-outline-yellow active:bg-yellow-600 transition ease-in-out duration-150" @endif>
                                                                            @if($emdadInvoice->invoice->invoice_status == 0) {{__('portal.Proforma Invoice Un-paid')}}
                                                                            @elseif($emdadInvoice->invoice->invoice_status == 1) {{__('portal.Proforma invoice verification Pending')}}
                                                                            @elseif($emdadInvoice->invoice->invoice_status == 2) {{__('portal.Proforma invoice rejected')}}
                                                                            @elseif($emdadInvoice->invoice->invoice_status == 3) {{__('portal.Proforma invoice accepted')}}
