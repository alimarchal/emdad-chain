            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="px-4 py-0 bg-white sm:p-6 rounded-sm">
                    <form action="{{route('adminIreUpdate')}}" method="post" class="form bg-white p-6  mb-4" enctype="multipart/form-data">
                        @csrf
                        <h3 class="text-2xl text-gray-900 font-semibold text-center">Edit IRE Information</h3>
                        <div class="flex space-x-5 mt-3">
                            <x-jet-label class="w-1/2" for="name">Name</x-jet-label>
                            <x-jet-label class="w-1/2" for="email">Email</x-jet-label>
                            <x-jet-label class="w-1/2" for="password">Password</x-jet-label>
...
{{--                            <x-jet-input id="password_confirmation" type="password_confirmation" name="password_confirmation" class="border p-2 w-1/2" value=""></x-jet-input>--}}
                        ...
                        <div class="flex space-x-5 mt-3">
                            <x-jet-label class="w-1/2" for="bank">Bank</x-jet-label>
                            <x-jet-label class="w-1/2" for="iban">IBAN</x-jet-label>
                            <x-jet-label class="w-1/2" for="nid_num">National ID #</x-jet-label>
                            <x-jet-label class="w-1/2" for="nid_image" title="File type: JPEG|PNG|PDF|DOCX => (Filesize: Max 10MB)">Upload National ID (max. 5 MB image)</x-jet-label>
                        </div>
                        <div class="flex space-x-5 mt-3">
...
{{--                            <x-jet-input id="nid_image" type="file" name="nid_image" class="border p-2 w-1/2"></x-jet-input>--}}
                        ...
                        <div class="flex space-x-5 mt-3">
                            ...
                            <x-jet-label class="w-1/2" for="type">Employee</x-jet-label>
                            <x-jet-label class="w-1/2" for="gender">Gender</x-jet-label>
                            <x-jet-label for="mobile_number" value="{{ __('Mobile Number') }}"  class="w-1/2"  />
                        </div>
                        <div class="flex space-x-5 mt-3">

                            <select name="type" id="type" required class="form-input rounded-md shadow-sm border p-2 w-1/2" value="{{ $ire->type }}">
                                <option disabled selected value="">Select</option>
                                <option {{ $ire->type == 0 ? "selected" : "" }} value="0">No</option>
                                <option {{ $ire->type == 1 ? "selected" : "" }} value="1">Yes</option>
                            </select>
                            <select name="gender" id="gender" required class="form-input rounded-md shadow-sm border p-2 w-1/2" value="{{ $ire->gender }}">
                                <option disabled selected value="">Select</option>
                                <option {{ $ire->gender == 0 ? 'selected' : '' }} value="0">Mr.</option>
                                <option {{ $ire->gender == 1 ? 'selected' : '' }} value="1">Ms.</option>
                                <option {{ $ire->gender == 2 ? 'selected' : '' }} value="1">Mrs.</option>
                            </select>
