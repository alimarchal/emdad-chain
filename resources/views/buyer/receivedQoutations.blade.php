@section('headerScripts')
    ...
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery.countdown/2.2.0/jquery.countdown.min.js" integrity="sha512-lteuRD+aUENrZPTXWFRPTBcDDxIGWe5uu0apPEn+3ZKYDwDaEErIK9rvR0QzUGmUQ55KFE2RqGTVoZsKctGMVw=="
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                @foreach ($PlacedRFQ as $placedRFQ)
                                    @if($placedRFQ->rfq_type == 1)
                                                    ...
                                                    @elseif($rfp->bypass == 0 && $rfp->qoutes->count() == 0 && $rfp->quotation_time < \Carbon\Carbon::now() && $rfp->status == 'pending')
                                                        @if(auth()->user()->can('Buyer View Quotations') || auth()->user()->hasRole('CEO'))
                                                            <a href="{{ route('resetQuotationTime', ['EOrderItemID' => $rfp->id]) }}"
                                                               class="inline-flex items-center justify-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-500 focus:outline-none focus:border-green-700 focus:shadow-outline-green active:bg-green-600 transition ease-in-out duration-150 confirm"
                                                               data-confirm='{{__('portal.Are you sure to reset this requisition?')}}'>
                                                                {{__('portal.Reset')}}
                                                            </a>
                                                    ...
                                                    @if($rfp->qoutes->count() > 0 && $rfp->quotation_time >= \Carbon\Carbon::now() && $rfp->bypass == 0)
                                                        ...
                                                        @if(auth()->user()->can('Buyer View Quotations') || auth()->user()->hasRole('CEO'))
                                                            <a href="{{ route('QoutationsBuyerReceivedQoutes', ['EOrderID' => $placedRFQ->id, 'EOrderItemID' => $rfp->id, 'bypass_id' => 1]) }}"
                                                               ...
                                                               data-confirm='{{__('portal.Once overrode you cannot receive quotations for this requisition')}}'>
                                                                {{__('portal.Override')}}
                                                            </a>
                                                        @endif
                                        ...
                                        @endforeach
                                    @else
{{--                                        {{dd($placedRFQ->OrderItems->count())}}--}}
                                        @if($loop->iteration <= $placedRFQ->OrderItems->count())
                                            <tr>
                                                <td class="px-6 py-4 text-center whitespace-nowrap">
                                            ...
                                            <tr>
                                                ...
                                                <td class="px-6 py-4 text-center whitespace-nowrap">
                                                    ...
                                                    @elseif($rfp->bypass == 0 && $rfp->qoutes->count() == 0 && $rfp->quotation_time < \Carbon\Carbon::now() && $rfp->status == 'pending')
                                                        @if(auth()->user()->can('Buyer View Quotations') || auth()->user()->hasRole('CEO'))
                                                            <a href="{{ route('resetQuotationTime', ['EOrderItemID' => $rfp->id]) }}"
                                                               class="inline-flex items-center justify-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-500 hover:text-white focus:outline-none focus:border-green-700 focus:shadow-outline-green active:bg-green-600 transition ease-in-out duration-150 confirm"
                                                               data-confirm='{{__('portal.Are you sure to reset this requisition?')}}'>
                                                                {{__('portal.Reset')}}
                                                            </a>
                                                            <a href="{{ route('discardQuotation', ['EOrderID' => $placedRFQ->id]) }}"
                                                               class="inline-flex items-center justify-center mt-2 px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500 hover:text-white focus:outline-none focus:border-red-700 focus:shadow-outline-red active:bg-red-600 transition ease-in-out duration-150 confirm"
                                                               data-confirm='{{__('portal.Are you sure to discard this requisition?')}}'>
                                                                {{__('portal.Discard')}}
                                                            </a>
                                                        @endif
