                ...
                <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg p-4">
                    ...
                    <div class="flex flex-wrap overflow-hidden xl:-mx-1 p-4 rounded shadow-md ">
                        <div class="w-full overflow-hidden lg:w-1/3 xl:my-1 xl:px-1 xl:w-1/3">
                            <strong>Quote Request #: {{ $QuoteItem->id }}</strong>
                        </div>

                        <div class="w-full overflow-hidden lg:w-1/3 xl:my-1 xl:px-1 xl:w-1/3">
                            <strong>Category Name:</strong> {{ $QuoteItem->orderItem->item_name }}
                        </div>

                        <div class="w-full overflow-hidden lg:w-1/3 xl:my-1 xl:px-1 xl:w-1/3">
                            <strong>Quote Quantity:</strong> {{ $QuoteItem->quote_quantity }}
                        </div>

                        <div class="w-full overflow-hidden lg:w-1/3 xl:my-1 xl:px-1 xl:w-1/3">
                            <strong>Quote Price Per Quantity: {{ $QuoteItem->quote_price_per_quantity }}</strong>
                        </div>
                        <div class="w-full overflow-hidden lg:w-1/3 xl:my-1 xl:px-1 xl:w-1/3">
                            <strong>Sample Information:</strong> {{ $QuoteItem->sample_information }}
                        </div>


                        <div class="w-full overflow-hidden lg:w-1/3 xl:my-1 xl:px-1 xl:w-1/3">
                            <strong>Sample Unit:</strong> {{ $QuoteItem->sample_unit }}
                        </div>
                        <div class="w-full overflow-hidden lg:w-1/3 xl:my-1 xl:px-1 xl:w-1/3">
                            <strong>Sample Security Charges:</strong> {{ $QuoteItem->sample_security_charges }}
                        </div>
                        <div class="w-full overflow-hidden lg:w-1/3 xl:my-1 xl:px-1 xl:w-1/3">
                            <strong>Sample Charges Per Unit:</strong> {{ $QuoteItem->sample_charges_per_unit }}
                        </div>
                        <div class="w-full overflow-hidden lg:w-1/3 xl:my-1 xl:px-1 xl:w-1/3">
                            <strong>Shipping Time In Days:</strong> {{ $QuoteItem->shipping_time_in_days }}
                        </div>
                        <div class="w-full overflow-hidden lg:w-1/3 xl:my-1 xl:px-1 xl:w-1/3">
                            <strong>Note:</strong> {{ strip_tags($QuoteItem->note_for_customer) }}
                        </div>
                        <div class="w-full overflow-hidden lg:w-1/3 xl:my-1 xl:px-1 xl:w-1/3">
                            <strong>Shipment Cost:</strong> {{ $QuoteItem->shipment_cost }}
                        </div>
                        <div class="w-full overflow-hidden lg:w-1/3 xl:my-1 xl:px-1 xl:w-1/3">
                            <strong>Vat (%):</strong> {{ $QuoteItem->VAT }}
                        </div>
                        <div class="w-full overflow-hidden lg:w-1/3 xl:my-1 xl:px-1 xl:w-1/3">
                            <strong>Total Cost:</strong> {{ $QuoteItem->total_cost }}
                        </div>
                    </div>

                    @if($QuoteItem->messages->isNotEmpty())
                        <div class="border-2 p-2 m-2">
                            @foreach ($QuoteItem->messages as $msg)
                    ...
                    <form action="{{ route('QuotationMessage.store') }}" class="rounded shadow-md" method="post">
                        @csrf
                        @php $business = \App\Models\Business::where('user_id', $QuoteItem->supplier_user_id)->first(); @endphp
                        ...
                        <input type="hidden" name="qoute_id" value="{{ $QuoteItem->id }}">
                        <input type="hidden" name="usertype" value="{{ $QuoteItem->business->business_type }}">
                    ...
                    <div class="justify-between p-2 m-2">
                        ...
                        <a href="{{ route('singleCategoryRFQUpdateStatusModificationNeeded', $QuoteItem->id) }}"
                        ...
                        <a href="{{ route('singleCategoryRFQUpdateStatusRejected', $QuoteItem->id) }}" style="margin-left: 70px; margin-top: 20px;"
                    ...
                    <br>
                    <form action="{{ route('qouteAccepted', $QuoteItem->id) }}" method="post">
                        @csrf
                        <input type="hidden" name="user_id" value="{{ auth()->user()->id }}">
                        <input type="hidden" name="business_id" value="{{ auth()->user()->business_id }}">

                        <input type="hidden" name="supplier_user_id" value="{{ $QuoteItem->supplier_user_id }}">
                        <input type="hidden" name="supplier_business_id" value="{{ $QuoteItem->supplier_business_id }}">

                        <input type="hidden" name="rfq_no" value="{{ $QuoteItem->e_order_id }}">
                        <input type="hidden" name="rfq_item_no" value="{{ $QuoteItem->e_order_items_id }}">

                        <input type="hidden" name="item_code" value="{{ $QuoteItem->orderItem->item_code }}">
                        <input type="hidden" name="item_name" value="{{ $QuoteItem->orderItem->item_name }}">

                        <input type="hidden" name="uom" value="{{ $QuoteItem->orderItem->unit_of_measurement }}">
                        <input type="hidden" name="brand" value="{{ $QuoteItem->orderItem->brand }}">

                        <input type="hidden" name="quantity" value="{{ $QuoteItem->quote_quantity }}">
                        <input type="hidden" name="unit_price" value="{{ $QuoteItem->quote_price_per_quantity }}">

                        <input type="hidden" name="sub_total" value="{{ $QuoteItem->quote_quantity * $QuoteItem->quote_price_per_quantity }}">
                        <input type="hidden" name="delivery_time" value="{{ $QuoteItem->shipping_time_in_days }}">

                        <input type="hidden" name="qoute_no" value="{{ $QuoteItem->id }}">

                        <input type="hidden" name="warehouse_id" value="{{ $QuoteItem->warehouse_id }}">
                        <input type="hidden" name="shipment_cost" value="{{ $QuoteItem->shipment_cost }}">
                        <input type="hidden" name="vat" value="{{ $QuoteItem->VAT }}">
                        <input type="hidden" name="total_cost" value="{{ $QuoteItem->total_cost }}">

                        <x-jet-label for="warehouse" class="my-2" value="{{ __('Warehouse delivery address') }}" class="text-black"  />

                        @php
                            $orderItemID =  \App\Models\EOrderItems::where('id', $QuoteItem->e_order_items_id)->first();
                        ...
                        <select name="payment_term" id="payment_term" class="form-input rounded-md shadow-sm border p-2 w-full" readonly>
                            @if ($QuoteItem->orderItem->payment_mode == 'Cash')
