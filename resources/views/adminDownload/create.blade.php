        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if($errors->any())
                <div class="block text-sm text-red-600 bg-red-200 border border-red-400 h-12 flex items-center p-4 rounded-sm relative" role="alert">
                    @if($errors->first('name_en'))<li><strong class="mr-1"> Name is required </strong></li> &nbsp;@endif
                    @if($errors->first('name_ar'))<li><strong class="mr-1"> Arabic Name is required </strong></li> &nbsp;@endif
                    @if($errors->first('icon'))<li><strong class="mr-1"> Icon must be of type: png, jpeg ,jpg </strong></li> &nbsp;@endif
                    @if($errors->first('file'))<li><strong class="mr-1"> File must be of type: excel, powerpoint or pdf </strong></li> @endif
                </div>
            @endif
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="px-4 py-0 bg-white sm:p-6 rounded-sm">
                    <form action="{{route('adminDownloadStore')}}" method="POST" class="form bg-white p-6  mb-4" enctype="multipart/form-data">
                        ...
                        <div class="flex space-x-5 mt-3">
                            <x-jet-label class="w-1/2" for="icon" title="File type: JPEG|PNG|PDF|DOCX => (Filesize: Max 10MB)">Icon</x-jet-label>
                            <x-jet-label class="w-1/2" for="file" title="File type: JPEG|PNG|PDF|DOCX => (Filesize: Max 10MB)">File Upload</x-jet-label>
                        </div>
                        <div class="flex space-x-5 mt-3">
