                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <div class="px-4 py-0 bg-white sm:p-6 rounded-sm">
                        <form action="{{route('adminDownloadUpdate')}}" method="POST" class="form bg-white p-6  mb-4" enctype="multipart/form-data">
                            ...
                            <div class="flex space-x-5 mt-3">
                                <x-jet-label class="w-1/2" for="icon" title="File type: JPEG|PNG|PDF|DOCX => (Filesize: Max 10MB)" required>{{__('portal.Icon')}}</x-jet-label>
                                <x-jet-label class="w-1/2" for="file" title="File type: JPEG|PNG|PDF|DOCX => (Filesize: Max 10MB)" required>{{__('portal.File Upload')}}</x-jet-label>
                            </div>
                            <div class="flex space-x-5 mt-3">
