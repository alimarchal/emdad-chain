                        ...
                        <form action="{{route('adminPercentageUpdate')}}" method="POST" class="form bg-white p-6  mb-4" enctype="multipart/form-data">
                            ...
                            <div class="flex space-x-5 mt-3">
                                ...
                                @if($commissionPercentage->commission_type == 1)
                                    <x-jet-label class="w-1/2" for="package_type" id="supplier_packages-label">{{__('portal.Package Type')}}</x-jet-label>
                                @elseif($commissionPercentage->commission_type == 2)
                                    <x-jet-label class="w-1/2" for="package_type" id="buyer_packages-label">{{__('portal.Package Type')}}</x-jet-label>
                                @endif
                            ...
                            <div class="flex space-x-5 mt-3">
                                <select name="commission_type" id="commission_type" onchange="packageType()" class="form-input rounded-md shadow-sm border p-2 w-1/2" value="{{ old('commission_type') }}" required>
                                    ...
                                    <option {{$commissionPercentage->commission_type == '0' ? 'selected' : ''}} value="0">{{__('portal.Sales')}}</option>
                                    <option {{$commissionPercentage->commission_type == '1' ? 'selected' : ''}} value="1">{{__('portal.Supplier')}}</option>
                                    <option {{$commissionPercentage->commission_type == '2' ? 'selected' : ''}} value="2">{{__('portal.Buyer')}}</option>
                                </select>

                                @if($commissionPercentage->commission_type == 1)
                                    <select id="supplier_packages" name="package_type" class="form-input rounded-md shadow-sm border p-2 w-1/2">
                                        <option disabled selected value="">{{__('portal.None')}}</option>
                                        <option {{$commissionPercentage->package_type == '1' ? 'selected' : ''}} value="1">{{__('portal.Basic')}}</option>
                                        <option {{$commissionPercentage->package_type == '2' ? 'selected' : ''}} value="2">{{__('portal.Silver')}}</option>
                                        <option {{$commissionPercentage->package_type == '3' ? 'selected' : ''}} value="3">{{__('portal.Gold')}}</option>
                                    </select>
                                @elseif($commissionPercentage->commission_type == 2)
                                    <select id="buyer_packages" name="package_type" class="form-input rounded-md shadow-sm border p-2 w-1/2">
                                        ...
                                        <option {{$commissionPercentage->package_type == '2' ? 'selected' : ''}} value="2">{{__('portal.Silver')}}</option>
                                        <option {{$commissionPercentage->package_type == '3' ? 'selected' : ''}} value="3">{{__('portal.Gold')}}</option>
                                        <option {{$commissionPercentage->package_type == '4' ? 'selected' : ''}} value="4">{{__('portal.Platinum')}}</option>
                                    </select>
                                @endif
                                ...
                                <select name="ire_type" id="ire_type" class="form-input rounded-md shadow-sm border p-2 w-1/2" value="{{ old('ire_type') }}" required>
                                    ...
                                    <option {{$commissionPercentage->ire_type == '0' ? 'selected' : ''}} value="0">{{__('portal.Non-Employee')}}</option>
                                    <option {{$commissionPercentage->ire_type == '1' ? 'selected' : ''}} value="1">{{__('portal.Employee')}}</option>
                                    <option {{$commissionPercentage->ire_type == '2' ? 'selected' : ''}} value="2">{{__('portal.Indirect Referral')}}</option>
                                </select>
                            </div>
                            <div class="flex space-x-5 mt-3">
                                <x-jet-label class="w-1/2" for="amount_type">{{__('portal.Amount Type')}}</x-jet-label>
                                @if($commissionPercentage->amount_type == 0)
                                    <x-jet-label class="w-1/2" for="amount" id="amount-label">{{__('portal.Amount')}}</x-jet-label>
                                @elseif($commissionPercentage->amount_type == 1)
                                    <x-jet-label class="w-1/2" for="percentage_amount" id="percentage_amount-label">{{__('portal.Amount %')}}</x-jet-label>
                                @endif

                                <x-jet-label class="w-1/2" for="amount" id="amount-label" style="display: none">{{__('portal.Amount')}}</x-jet-label>
                                <x-jet-label class="w-1/2" for="percentage_amount" id="percentage_amount-label" style="display: none">{{__('portal.Amount %')}}</x-jet-label>
                            ...
                            <div class="flex space-x-5 mt-3" required>
                                <select name="amount_type" id="amount_type" class="form-input rounded-md shadow-sm border p-2 w-1/2" onchange="disable()" value="{{ old('amount_type') }}" required>
                                    <option disabled selected value="">{{__('portal.None')}}</option>
                                    <option {{$commissionPercentage->amount_type == '0' ? 'selected' : ''}} value="0">{{__('portal.Amount')}}</option>
                                    <option {{$commissionPercentage->amount_type == '1' ? 'selected' : ''}} value="1">{{__('portal.Percentage')}}</option>
                                </select>

                                @if($commissionPercentage->amount_type == 0)
                                    <x-jet-input id="amount" type="number" name="amount" class="border p-2 w-1/2" value="{{ $commissionPercentage->amount  }}" step="1" min="0" placeholder="{{__('portal.Enter amount')}}"></x-jet-input>
                                @elseif($commissionPercentage->amount_type == 1)
                                    <x-jet-input id="percentage_amount" type="number" name="percentage_amount" class="border p-2 w-1/2" value="{{ $commissionPercentage->amount * 100  }}" step="0.01" min="1" placeholder="{{__('portal.Enter amount in percentage')}}"></x-jet-input>
                                @endif
